...
public class WorkerPool {
	...
	public static void main(String args[]) throws InterruptedException { 
		...
		while ( true ) {
		  ...
		  System.out.println(new Timestamp((new java.util.Date()).getTime()) + " - Consumed: " + item + " from Hazelcast Task Queue");
		  executorPool.execute(new WorkerThread(processTime,item,retrySleepTime,retryMaxAttempts));
		  taskNumber++;
		  if ( "-1".equals(item) ) {
			System.out.println(new Timestamp((new java.util.Date()).getTime()) + " - Detected end of Task identifier");
			queue.put( "-1" );
			break;
		  }
		}
		System.out.println(new Timestamp((new java.util.Date()).getTime()) + " - Hazelcast consumer Finished" );
/*
